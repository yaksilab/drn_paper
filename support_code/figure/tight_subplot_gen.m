...
for ih = 1:n_row
    px = mar_sid(1);
    for ix = 1:n_col
        pan = pan + 1;% rows, then columns
        hax_sub(pan) = axes('Units', 'normalized', 'Position', [px y_pos wid_axe(pan) hei_axe(pan)], ...
            'XTickLabel', '', 'YTickLabel', '');
        px = px + wid_axe(pan) + gap_hor_row_col(ih, ix);
    end
    %y_pos = y_pos - hei_ori_axe(ii) - gap_ver_row(ih);
    
    if pan < n_plo
        y_pos = y_pos - hei_axe(pan + 1) - gap_ver_row(ih);
    end
