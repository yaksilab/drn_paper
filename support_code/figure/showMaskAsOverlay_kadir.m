function varargout = showMaskAsOverlay_kadir(opacity, mask, overlaycolor, varargin)
...
if nargin >= 4
    ...
    %axes(imgax);% kadir: comment if not doing cell detection
else
    fig = gcf;
end
...
%figure(fig);% kadir: comment if not doing cell detection
tmp = imhandles(fig);
if isempty(tmp)
...
% h = imshow(newmask);
hold(fig, 'on')
h = imshow(newmask, 'Parent', fig);%kadir: if not doing cell detection, replace above
try
