function [H, y_lim] = sigstar_lim(hax, groups, fon_siz_non, off_non, y_lim_pre, sep, mou, ...
    ext_hei, stats, nosort)
...
%-----------------------------------------------------
    ...
    for ii=1:n_gro
        ...
        thi_gro(ii) = thisY;
        upp_lim_gro(ii) = ext_upp_lim(thisY, stats(ii), y_lim_pre, off_non, sep, mou, ext_hei);
    end
    ...
    % printing
    for ii=1:length(groups)
        H(ii,:) = makeSignificanceBar(xlocs(ii,:), thi_gro(ii), stats(ii), fon_siz_non, ...
            off_non, y_lim_pre);
    end
    %-----------------------------------------------------
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Internal functions
function upp_lim = ext_upp_lim(y, p, y_lim_pre, off_non, sep, mou, ext_hei)
    if p <= 0.05
        offset = 0.01;
    else
        offset = off_non;
...
function H = makeSignificanceBar(x,y,p, fon_siz_non, off_non, y_lim_pre)
    n_dig = 2;
    x=repmat(x,2,1);
    y=repmat(y,4,1);
    %thr = 0.07;
    ...
    if p <= 0.05
        ...
        H(2) = text(mean(x(:)), starY, stars, 'Units', 'data', 'HorizontalAlignment', 'Center', ...
            'BackGroundColor', 'none', 'Tag', 'sigstar_stars');
    elseif p <= thr
        offset = off_non;
        starY = mean(y) + myRange(y_lim_pre)*offset;
        H(2) = text(mean(x(:)), starY, stars, 'Units', 'data', 'HorizontalAlignment', 'Center', ...
            'BackGroundColor', 'none', 'Tag', 'sigstar_stars', 'FontSize', fon_siz_non);
    else
        H = nan;
    end
end
