...
% edgecolor: LineColor. (default 'k'); Specify abbrev. (e.g. 'k' for black); set either [],'' or 
% 'none' if the mean should not be plotted
...
%}
%%
function[h,L,MX,MED,bw]=violin(hax, Y,varargin)
...
for i=1:size(Y,2)
    var_uni = Y{i};
    var_uni = var_uni(~isoutlier(var_uni, 'percentiles', [10 90]));
    if isempty(b)==0
        [f, u, bb]=ksdensity(var_uni, 'bandwidth', b(i));
    elseif isempty(b)
        [f, u, bb]=ksdensity(var_uni);
    end
    f=f/max(f)*0.3; %normalize
    F(:,i)=f;
    U(:,i)=u;
    MED(:,i)=nanmedian(var_uni);
    MX(:,i)=nanmean(var_uni);
    bw(:,i)=bb;
end
%%
%-------------------------------------------------------------------------
...
for i=i:size(Y,2)
    if isempty(lc) == 1
        if setX == 0
            h(i)=fill(hax, [F(:,i)+i;flipud(i-F(:,i))],[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor','none');
        else
            h(i)=fill(hax, [F(:,i)+x(i);flipud(x(i)-F(:,i))],[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor','none');
        end
    else
        if setX == 0
            h(i)=fill(hax, [F(:,i)+i;flipud(i-F(:,i))],[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor',lc);
        else
            h(i)=fill(hax, [F(:,i)+x(i);flipud(x(i)-F(:,i))],[U(:,i);flipud(U(:,i))],fc(i,:),'FaceAlpha',alp,'EdgeColor',lc);
        end
    end
      hold(hax, 'on')
end
