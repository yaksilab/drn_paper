classdef ind_sig
    %IND_SIG - One line description of what the class performs (H1 line)
    %   Optional file header info (to give more details about the function than in the H1 line)
    %   Optional file header info (to give more details about the function than in the H1 line)
    %   Optional file header info (to give more details about the function than in the H1 line)
    %
    %   IND_SIG constructor syntax:
    %       IND_SIG_OBJ = IND_SIG()
    %
    %   IND_SIG properties/attributes:
    %       Property 1 - description
    %       Property 2 - description
    %
    %   IND_SIG methods/functions:
    ...
    %   Date: 14-Dec-2022; Last revision: 14-Dec-2022
    %
    ...
    methods(Static)
        function [H, y_lim] = sig(hax, groups, y_lim_pre, sep, mou, ext_hei, stats, nosort)
        ...
        %-----------------------------------------------------
            ...
            yd=myRange(y_lim_pre)*sep;
            %
            n_gro = length(groups);
            upp_lim_gro = nan(n_gro, 1);
            thi_gro = nan(n_gro, 1);
            for ii=1:n_gro
                % thisY: y for the horizontal line ---- rec for next
                x_ind_i = xlocs(ii,:);
                if diff(x_ind_i) == 1
                    thisY = findMinY(x_ind_i) + yd;
                else
                    thisY = max([findMinY(x_ind_i); thi_gro]) + yd;
                end
                thi_gro(ii) = thisY;
                upp_lim_gro(ii) = ext_upp_lim.sig(thisY, stats(ii), y_lim_pre, sep, mou, ext_hei);
            end
            y_lim = [y_lim_pre(1) max(upp_lim_gro)];
            ylim(y_lim) % !!!!!!!!!! changed only here!
            %
            % printing
            ...
            for ii=1:length(groups)
                H(ii,:) = makeSignificanceBar.sig(xlocs(ii,:), thi_gro(ii), stats(ii), y_lim_pre, offset);
            end
            %-----------------------------------------------------
            ...
            %yd = myRange(ylim)*0.03;
            yd = myRange(y_lim_pre)*mou;
            for ii = 1:length(groups)
                if ~isnan(H(ii,1))
                    y = get(H(ii,1),'YData');
                    y(1) = y(1) - yd;
        ...
        function [H, y_lim] = duo(hax, groups, fon_siz_non, off_non, y_lim_pre, sep, mou, ...
            ext_hei, stats, stats_two, nosort)
            % sigstar - Add significance stars to bar charts, boxplots, line charts, etc,
            %
            % H = sigstar(groups,stats,nsort)
            %
        ...
        %-----------------------------------------------------
            ...
            for ii=1:n_gro
                ...
                thi_gro(ii) = thisY;
                upp_lim_gro(ii) = ext_upp_lim.duo(thisY, stats(ii), y_lim_pre, off_non, sep, mou, ext_hei);
            end
            y_lim = [y_lim_pre(1) max(upp_lim_gro)];
            ylim(y_lim) % !!!!!!!!!! changed only here!
            %
            % printing
            for ii=1:length(groups)
                H(ii,:) = makeSignificanceBar.duo(xlocs(ii,:), thi_gro(ii), stats(ii), fon_siz_non, ...
                    off_non, y_lim_pre);
                makeSignificanceBar.two(xlocs(ii,:), thi_gro(ii), stats_two(ii), fon_siz_non, off_non, ...
                    y_lim_pre)
            end
            %-----------------------------------------------------
            %Now we can add the little downward ticks on the ends of each line. We are
        ...
        function pri_sig(hax, nor_x, nor_y, p_val)
