...
handles.bar = bar(varargin{:}, 'LineWidth', lin_wid); % standard implementation of bar fn
hold on
hBar = handles.bar;
if nRows > 1
    ...
    for col = 1:nCols
        ...
        hErrorbar(col) = errorbar(mean(x,1), values(xOrder,col), lowerErrors(xOrder,col), upperErrors(xOrder, col), '.k', 'LineWidth', lin_wid);
    ...
    hErrorbar = errorbar(mean(x,1), values, lowerErrors, upperErrors, '.k', 'LineWidth', lin_wid);
