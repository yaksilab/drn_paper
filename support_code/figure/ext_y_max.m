function [y_lim, thi_gro] = ext_y_max(hax, groups, fon_siz_non, off_non, y_lim_pre, sep, mou, ...
...
function upp_lim = ext_upp_lim(y, p, y_lim_pre, off_non, sep, mou, ext_hei)
    ...
    Y = ylim;
    if Y(2) < starY
        %upp_lim = starY + myRange(Y)*(sep + mou + 0.030);
...
function H = makeSignificanceBar(x,y,p, fon_siz_non, off_non, y_lim_pre)
    ...
    elseif p<=0.05
        ...
        %stars = strcat("p=", string(round(p, n_dig)));
        stars = string(round(p, n_dig));
    end
    if p <= 0.05
        offset = 0.01;
        starY = mean(y) + myRange(y_lim_pre)*offset;
        H(2) = text(mean(x(:)), starY, stars, 'Units', 'data', 'HorizontalAlignment', 'Center', 'BackGroundColor', 'none', 'Tag', 'sigstar_stars');
    else
        offset = off_non;
        starY = mean(y) + myRange(y_lim_pre)*offset;
        H(2) = text(mean(x(:)), starY, stars, 'Units', 'data', 'HorizontalAlignment', 'Center', 'BackGroundColor', 'none', 'Tag', 'sigstar_stars', ...
        'FontSize', fon_siz_non);
    end
end
