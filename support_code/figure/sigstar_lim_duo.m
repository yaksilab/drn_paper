function [H, y_lim] = sigstar_lim_duo(hax, groups, fon_siz_non, off_non, y_lim_pre, sep, mou, ...
...
%-----------------------------------------------------
    ...
    for ii=1:length(groups)
        H(ii,:) = makeSignificanceBar(xlocs(ii,:), thi_gro(ii), stats(ii), fon_siz_non, ...
            off_non, y_lim_pre);
        makeSignificanceBar_two(xlocs(ii,:), thi_gro(ii), stats_two(ii), fon_siz_non, off_non, ...
            y_lim_pre)
    end
    %-----------------------------------------------------
...
function makeSignificanceBar_two(x,y,p, fon_siz_non, off_non, y_lim_pre)
    n_dig = 2;
    x=repmat(x,2,1);
    y=repmat(y,4,1);
    %thr = 0.07;
