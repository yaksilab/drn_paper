        ...
        for j=0:ncols-1
            ...
            if plottype>=2
                ...
                switch plottype
                    case 2
                        hold on
                        scatter(spt_curr,-alpha*(spc_curr-1)+2,20,...
                            colord(mod(spc_curr-1,size(colord,1))+1,:),'o','filled')
                        hold off
                    case 3
                        cla
                        scatter(spt_curr,-alpha*(spc_curr-1)+0.4*alpha,20,...
                            colord(mod(spc_curr-1,size(colord,1))+1,:),'o','filled')
                    ...
                    case 4
                        ...
                        for jj=1:length(ICuseuse)
                            ...
                            errorbar(tvec,spr,sperr,'Color',colord(mod(jj-1,size(colord,1))+1,:),...
                                'LineWidth',1)
                            hold on
                            bar(tvec,spr)
                            hold off
                        ...
                        sperr = sqrt(spn)./(binsize*length(ICuseuse));  
                        %Standard error for measurement of mean from Poisson process
                    ...
                    case 5
                        ...
                        for jj = 1:length(nsp_t)
                            plot( nsp_t(jj)*ones(nsp(jj),1), spc_curr(spt_curr==nsp_t(jj)), 'o-', ...
                                'Color', colord(mod(nsp(jj)-2,size(colord,1))+1,:), 'LineWidth', 2, ...
                                'MarkerSize',10)
                            hold on
                        end
                        scatter(spt_curr,-spc_curr,20,colord(mod(spc_curr-1,size(colord,1))+1,:),'o',...
                            'filled')
                        hold off
        ...
        if plottype>=2
            ...
            switch plottype
                case 2
                    % Traces with spikes
                    hold on
                    scatter(spt,-alph*(spc-1)+0.4*alph,20,colord(mod(spc-1,size(colord,1))+1,:),'o',...
                        'filled')
                    hold off
                case 3
                    % Spike raster only
                ...
                case 4
                    ...
                    patch([tvec(end:-1:1),tvec],[mspr(end:-1:1)+msperr(end:-1:1),mspr-msperr],...
                        0.5*[1,1,1],'FaceAlpha',1)
                    hold off
                    axis tight
                    formataxes
            end
