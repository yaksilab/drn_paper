function combined_y_pos = AO_average_y_trace_flexibel(all_fish, n_groups, groups, names, cmap, ...
    folder_path_save, figures_subfolder, start_point,end_point, seperate_y, stimulus)
...
% folder_path_save, figures_subfolder, start_point,end_point, seperate_y, stim)
...
%   AO_plot_ypos_average_and_change (plots the average and the change over each stimulus of one 
% conditions)
%   Author: Anna Maria Ostenrath 
%   Date: May 2022	
    ...
    for group = 1:n_groups 
       if ismember(fish,groups{group})
           if ismember(fish,lowerRow)
               ...
               collected_y_pos_lower{group}(:, fish) = temp_y(start_time:end_time)+168; 
               % adding a value to have the "true zero"
           else
               y = all_fish{fish,1}.y;
               temp_y = -y;
               collected_y_pos_upper{group}(:, fish) = temp_y(start_time:end_time)+26; 
               % adding a value to have the "true zero"
           end
       end 
    end
...
if seperate_y
    ...
    %
    for group = 1:n_groups 
        p1 = shadedErrorBar(x_values, squeeze(nanmean(collected_y_pos_upper{group,1},2)),squeeze...
            (nanstd(collected_y_pos_upper{group,1}(:,:),0,2)/sqrt(sizes_up{group})), 'lineProps','k');
...
%         p1.edge.Color = cmap(group,:);
    ...
    hold on 
    for group = 1:n_groups 
        p1 = shadedErrorBar(x_values, squeeze(nanmean(collected_y_pos_lower{group},2)),squeeze...
            (nanstd(collected_y_pos_lower{group}(:,:),0,2)/sqrt(sizes_low{group})), 'lineProps','k');
        p1.mainLine.Color = cmap(group,:);
        p1.patch.FaceColor = cmap(group,:); 
%         p1.edge.Color = cmap(group,:);
        all_plots = [all_plots, p1]; 
    ...
    saveas(gcf, fullfile(folder_path_save, figures_subfolder, ...
        ['Y_pos_comparison_groups_seperated_rows.png']));
    % close; 
end
...
x_values = t(start_time:end_time);
hold on
all_plots = [];
for group = 1:n_groups 
    H1=shadedErrorBar(x_values,squeeze(nanmean(combined_y_pos{group},2)),squeeze...
        (nanstd(combined_y_pos{group}(:,:),0,2)/sqrt(size(groups{group},1))), 'lineProps','k');
    H1.mainLine.Color = cmap(group,:);
    H1.patch.FaceColor = cmap(group,:); 
    H1.mainLine.LineWidth = 2; 
    all_plots = [all_plots, H1];
end
if stimulus
    hold on
    for stim = 1:length(stim_times)
...
if stimulus
    saveas(gcf, fullfile(folder_path_save, figures_subfolder, ...
        ['Y_pos_comparison_groups_stim_period.png']));
else
    ...
    xline(t(late(1)))
    xline(t(late(end)))
    pfe(h_fig, las_pix, rat_wid)
    exp_fig(h_fig, fullfile(folder_path_save, figures_subfolder, ['Y_pos_comparison_groups.png']));
end
...
%     late = find(t>end_point-time_duration & t<=end_point); 
    ...
    if n_groups == 2
        figure
        a = scatter(ones(length(avg_early_1),1)*0.5, avg_early_1, 'filled', 'MarkerEdgeColor', ...
            cmap(1,:), 'MarkerFaceColor', cmap(1,:)); 
        hold on
        b = scatter(ones(length(avg_early_2),1)*1, avg_early_2, 'filled',  'MarkerEdgeColor', ...
            cmap(2,:), 'MarkerFaceColor', cmap(2,:));
        scatter(ones(length(avg_late_1),1)*2.5, avg_late_1, 'filled',  'MarkerEdgeColor', ...
            cmap(1,:), 'MarkerFaceColor', cmap(1,:));
        scatter(ones(length(avg_late_2),1)*3, avg_late_2, 'filled',  'MarkerEdgeColor', ...
            cmap(2,:), 'MarkerFaceColor', cmap(2,:));
            er = errorbar([0.5 1 2.5 3],[nanmean(avg_early_1), nanmean(avg_early_2), ...
                nanmean(avg_late_1), nanmean(avg_late_2)],[sem_Early1, sem_Early2,sem_Late1,...
                sem_Late2],[-sem_Early1, -sem_Early2, -sem_Late1,-sem_Late2]);    
        er.Color = [0 0 0];                            
        er.LineStyle = 'none';  
        xlim([0 4])
        ylim([-105 0])
        ...
        [p, h] = quick_statistic(avg_early_1(:,groups{1,1}), avg_early_2(:,groups{2,1}))
        [p, h] = quick_statistic(avg_late_1, avg_late_2)

        [p, h] = quick_statistic_signrank(avg_early_1, avg_late_1)
        [p, h] = quick_statistic_signrank(avg_early_2, avg_late_2)
    ...
    elseif n_groups == 3
        ...
        b = scatter(ones(length(avg_early_2),1)*1, avg_early_2, 'filled',  'MarkerEdgeColor', ...
            cmap(2,:), 'MarkerFaceColor', cmap(2,:));
        c = scatter(ones(length(avg_early_3),1)*1.5, avg_early_3, 'filled',  'MarkerEdgeColor', ...
            cmap(3,:), 'MarkerFaceColor', cmap(3,:));

        scatter(ones(length(avg_late_1),1)*2.5, avg_late_1, 'filled',  'MarkerEdgeColor', ...
            cmap(1,:), 'MarkerFaceColor', cmap(1,:));
        scatter(ones(length(avg_late_2),1)*3, avg_late_2, 'filled',  'MarkerEdgeColor', ...
            cmap(2,:), 'MarkerFaceColor', cmap(2,:));
        scatter(ones(length(avg_late_3),1)*3.5, avg_late_3, 'filled',  'MarkerEdgeColor', ...
            cmap(3,:), 'MarkerFaceColor', cmap(3,:));
        
        % crash -kadir
%         er = errorbar([0.5 1 1.5 2.5 3 3.5],[nanmean(avg_early_1), nanmean(avg_early_2), nanmean...
%             (avg_early_3), nanmean(avg_late_1), nanmean(avg_late_2), nanmean(avg_late_3)],...
%             [sem_Early1, sem_Early2, sem_Early3, sem_Late1,sem_Late2, sem_Late3],...
%             [-sem_Early1, -sem_Early2,-sem_Early3, -sem_Late1,-sem_Late2, -sem_Late3]);    
%         er.Color = [0 0 0];                            
%         er.LineStyle = 'none';  
        xlim([0 4])
        ...
        plot([0.5, 1, 1.5], [nanmean(avg_early_1), nanmean(avg_early_2),nanmean(avg_early_3)], ...
            'Color', 'k')
        plot([2.5, 3, 3.5], [nanmean(avg_late_1), nanmean(avg_late_2),nanmean(avg_late_3)], ...
            'Color', 'k')
        legend([a b c], names)
    ...
    [p, h] = quick_statistic(avg_early_1, avg_early_2)
    [p, h] = quick_statistic(avg_early_1, avg_early_3)
    [p, h] = quick_statistic(avg_early_2, avg_early_3)
    pfe(h_fig, las_pix, rat_wid)
    exp_fig(h_fig, fullfile(folder_path_save, figures_subfolder, ['Y_pos_scatter_plot.png']));
end
end
