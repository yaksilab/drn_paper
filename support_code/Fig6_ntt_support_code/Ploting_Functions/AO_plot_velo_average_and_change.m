function [collected_velo,change_velo, collected_stim] = AO_plot_velo_average_and_change(all_fish, ...
    group,name, duration, timebin, folder_path_save, figures_subfolder,single_ploting)
...
%
%   Syntax:
%       [collected_velo,change_velo, collected_stim] = AO_plot_velo_average_and_change(all_fish, ...
% group,name, duration, timebin, folder_path_save, figures_subfolder,single_ploting)
%    
%   Inputs:
...
stim_times = all_fish{group(1), 1}.timeStampStim(find(~contains(all_fish...
    {group(1), 1}.stimInfo, "VIB_0")));
...
len_velo_trace = length(fish_speed(new_time>floor(stim_times(1))-duration & new_time<floor...
    (stim_times(1))+duration));  % double check why you floor it
collected_velo = nan([len_velo_trace, size(group,1), length(stim_times)]);
collected_stim = [];
change_velo = nan(size(collected_velo)); 
baseline_sec = 7; % the baseline duration in seconds% kadir
for stim = 1: length(stim_times)
   for fish = 1:size(group,1)
       ...
       curr_stim_times = all_fish{group(fish), 1}.timeStampStim(find(~contains(all_fish{group...
           (fish), 1}.stimInfo, "VIB_0")));
       curr_stim_ons = curr_stim_times(stim); 

       temp_speed = fish_speed(new_time>floor(curr_stim_ons)-duration & new_time<floor(curr_stim_ons)...
           +duration); 
       if size(temp_speed,2) < len_velo_trace
           temp_speed = fish_speed(new_time>floor(curr_stim_ons)-duration & new_time<floor...
               (curr_stim_ons)+duration+1);
       end
      ...
      if group(fish) == group(1)
       ...
       change_velo(:, fish, stim) = temp_speed(1:len_velo_trace)-nanmean(temp_speed(marker_stim-round...
           (baseline_sec/timebin):marker_stim),2); 
   end
...
for stim = 1:length(stim_types)
    ...
    subplot(1,3,stim)
    H=shadedErrorBar(x_values, squeeze(nanmean(nanmean(collected_velo(:,:,current_stim_ind),3),2)),...
        squeeze(nanstd(nanmean(collected_velo(:,:,current_stim_ind),3),0,2)/sqrt(size(group,1))), ...
        'lineProps','k');
    hold on
...
for stim = 1:length(stim_types)
    current_stim_ind = find(stim_types_indice == stim_types(stim)); 
    subplot(1,3,stim)
    H=shadedErrorBar(x_values, squeeze(nanmean(nanmean(change_velo(:,:,current_stim_ind),3),2)),...
        squeeze(nanstd(nanmean(change_velo(:,:,current_stim_ind),3),0,2)/sqrt(size(group,1))), ...
        'lineProps','k');
    hold on
    xline(collected_stim(stim), 'Color', 'r' )
    hold off
...
saveas(gcf, fullfile(folder_path_save, figures_subfolder, name, ...
    'Average_velo_change_per_stim_avg_fish.png')); 
close
if single_ploting
    for fish = 1:size(group,1)
            figure('units','centimeters','Position',[2 2 20 8])
            ...
            for stim = 1:length(stim_types)
                current_stim_ind = find(stim_types_indice == stim_types(stim)); 
                subplot(1,3,stim)
                H=shadedErrorBar(x_values, squeeze(nanmean(collected_velo(:,fish,current_stim_ind),...
                    3)),squeeze(nanstd(collected_velo(:,fish,current_stim_ind),0,3)/sqrt(length(...
                    current_stim_ind))), 'lineProps','k');
                hold on
                xline(collected_stim(stim), 'Color', 'r' )
                hold off
                ylim([0 5])
            ...
            saveas(gcf, fullfile(folder_path_save, figures_subfolder, name, ['Fish_',int2str(fish), ...
                '_average_velo_per_stim.png']));
            close
    end
end
end
