function p_val = AO_average_y_trace_flexibel_sca(all_fish, n_groups, groups, names, cmap, ...
    folder_path_save, figures_subfolder, start_point,end_point, seperate_y, stimulus, win_siz_min)
...
%         p1.edge.Color = cmap(group,:);
    ...
    xlabel('Time [s]')
    legend(names)
    p_val = com_nov(t, end_point, collected_y_pos_upper, groups, cmap, names, folder_path_save, ...
        figures_subfolder);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    subplot(2,1,2)
    all_plots = []; 
    ...
    xlabel('Time [s]')
    legend(names)
    p_val = com_nov(t, end_point, collected_y_pos_lower, groups, cmap, names, folder_path_save, ...
        figures_subfolder);
...
p_val = com_nov(t, end_point, combined_y_pos, groups, cmap, names, folder_path_save, ...
    figures_subfolder, win_siz_min);
end

function p_val = com_nov(t, end_point, combined_y_pos, groups, cmap, names, folder_path_save, ...
    figures_subfolder, win_siz_min)
% this part is for ploting just the avg y pos early and late
time_duration = win_siz_min*60;
...
figure
a = scatter(ones(length(avg_early_1),1)*0.5, avg_early_1, 'filled', 'MarkerEdgeColor', cmap(1,:), ...
    'MarkerFaceColor', cmap(1,:)); 
hold on
b = scatter(ones(length(avg_early_2),1)*1, avg_early_2, 'filled',  'MarkerEdgeColor', cmap(2,:), ...
    'MarkerFaceColor', cmap(2,:));
scatter(ones(length(avg_late_1),1)*2.5, avg_late_1, 'filled',  'MarkerEdgeColor', cmap(1,:), ...
    'MarkerFaceColor', cmap(1,:));
scatter(ones(length(avg_late_2),1)*3, avg_late_2, 'filled',  'MarkerEdgeColor', cmap(2,:), ...
    'MarkerFaceColor', cmap(2,:));

er = errorbar([0.5 1 2.5 3],[nanmean(avg_early_1), nanmean(avg_early_2), nanmean(avg_late_1), ...
    nanmean(avg_late_2)],[sem_Early1, sem_Early2,sem_Late1,sem_Late2],[-sem_Early1, -sem_Early2, ...
    -sem_Late1,-sem_Late2]);    
er.Color = [0 0 0];                            
er.LineStyle = 'none';
xlim([0 4])
...
[p, h, p_xsa] = quick_statistic(avg_early_1(:,groups{1,1}), avg_early_2(:,groups{2,1}));
p_val.rst = p(2);
p_val.lme = p_xsa;
...
saveas(gcf, fullfile(folder_path_save, figures_subfolder, 'Y_pos_scatter_plot.png'));
end