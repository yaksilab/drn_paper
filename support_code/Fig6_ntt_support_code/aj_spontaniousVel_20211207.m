function aj_spontaniousVel_20211207 (all_fish,timebin,folder_path_save,fish_name,...
...
f1 = tiledlayout(length(groups)-2,1);
...
for i=1: length(groups)-2
    ...
    xlabel(['Mean velocity (mm/s), ',name_groups{1,i},...
        ' (',num2str(timebin),' sec time bins)']);
    ylabel('Fish number');
    colormap (flipud(hot))
    colorbar;
    set(gca, 'CLim', [0 10]);
...
for i=1:length(groups)-2
    ...
    set(b,{'BoxFaceColor'},{C1(1,:); C1(2,:) ;C1(3,:)}) 
...
saveas (f17, [folder_path_save,filesep,figures_subfolder,filesep,fish_name,date_exp '_sponBehaviour_meanVel_test' '.pdf']);
