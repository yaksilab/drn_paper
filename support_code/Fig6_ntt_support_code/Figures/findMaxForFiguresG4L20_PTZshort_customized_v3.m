...
for i= 1%:size(idxLightsOn,2)/5
    nexttile
    Vec = ISI-latency';
    b= boxchart(ISI_amplitudes([group1 group2 group3 group4],:),...
        'BoxFaceAlpha',0.5,'LineWidth',.5,'MarkerStyle','none','GroupByColor',group_category([group1 group2 group3 group4],:));
    set(b,{'BoxFaceColor'},C1)
    
    h = text(1,90,{num2str(ranksum(ISI_amplitudes(group1),ISI_amplitudes(group2))) ;...           % change the number of test done manually
            num2str(ranksum(ISI_amplitudes(group1),ISI_amplitudes(group3))) ;...
            num2str(ranksum(ISI_amplitudes(group1),ISI_amplitudes(group4)))}) ;...
%             num2str(ranksum(Vec(group1),Vec(group2))) ]);
 set(h,'Rotation',270);
