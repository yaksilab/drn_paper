...
%     Vec{i,1}(:,j) = ([peakAnalysisCell_perISI{i, 1}.latency]);                         % something is wrong works only for the 5 min ISI
    ...
    %Vec2 = mean(Vec{i,1},2);
    b= boxchart(Vec([groups{1,1:2}],:),'BoxWidth',.5,'BoxFaceAlpha',0.5,'LineWidth',.5,'MarkerStyle','none',...
        'GroupByColor',group_category([groups{1,1:2}],:));
    set(b,{'BoxFaceColor'},C1(1:2))
...
%     h = text(1,10,{num2str(ranksum(Vec(group1),Vec(group2)))});          % change the number of test done manually
%         num2str(ranksum(Vec(group1),Vec(group3)))});
% %         num2str(ranksum(Vec(group1),Vec(group4)))});
% set(h,'rotation',270)
ylim([0 10]);
...
%     h = text(1,8,{num2str(ranksum(Vec(group1),Vec(group2)))});         % change the number of test done manually
%         num2str(ranksum(Vec(group1),Vec(group3)))});
% set(h,'rotation',270)
ylim([0 10]);
yticks([0 5 10]);
