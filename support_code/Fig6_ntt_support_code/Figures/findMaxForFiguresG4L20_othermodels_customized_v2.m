...
%% Calc mean velocity per ISI

tempVelISI=cell(size(idxLightsOn,2),1);
for r=1:size(idxLightsOn,2)
    tempVelISI{r,1}=tempMat{r,6};
end
VelISI_3Dmat=cell2mat(reshape(tempVelISI,1,1,[])); %convert to 3D matrix
VelISI_3Dmat_meanISI5=mean(VelISI_3Dmat(:,:,6:10),3); %calc mean ISI5 (1:5 for PTZ_short / 6:10 for other models!)

peakAnalysisCell_perISI=cell(1,1); %change if includeds more ISI
nROIs = length(all_fish);
for i=1%:size(idxLightsOn,2)
    for ROI=1:nROIs
        peakAnalysisCell_perISI{i, 1}(ROI,1).amplitude = ...
            max(VelISI_3Dmat_meanISI5(ROI,13:24)-mean(VelISI_3Dmat_meanISI5(ROI,1:12),...
            'omitnan'));
        peakAnalysisCell_perISI{i, 1}(ROI,1).latency = (find...
            ((VelISI_3Dmat_meanISI5(ROI,13:24)-mean(VelISI_3Dmat_meanISI5(ROI,1:12)))...
            == max((VelISI_3Dmat_meanISI5(ROI,13:24))-mean(VelISI_3Dmat_meanISI5...
            (ROI,1:12)))))*timebin;
        peakAnalysisCell_perISI{i,1}(ROI,1).latency = ...
            peakAnalysisCell_perISI{i,1}(ROI,1).latency(1,1)*timebin;
        %          = findpeaks(diff(VelISI_3Dmat_meanISI5(ROI,12:24)),'Npeaks',1,...
        %          'WidthReference', 'halfheight');
    end
end 
...
for i= 1%:size(idxLightsOn,2)
    for ROI= 1:nROIs
        if isempty (peakAnalysisCell_perISI{i, 1}(ROI,1).amplitude)
            peakAnalysisCell_perISI{i, 1}(ROI,1).amplitude = nan;
            peakAnalysisCell_perISI{i, 1}(ROI,1).latency = nan;
        end
    end
end


peakAnalysisCell_perISIOff = cell (1,1); % for DVV_trad
for i=1%:size(idxLightsOn,2)
    for ROI=1:nROIs
        peakAnalysisCell_perISIOff{i, 1}(ROI,1).amplitude =...
            max(VelISI_3Dmat_meanISI5(ROI,37:48)-mean(VelISI_3Dmat_meanISI5(ROI,35:36),...
            'omitnan'));
        peakAnalysisCell_perISIOff{i,1}(ROI,1).latency =...
            find(VelISI_3Dmat_meanISI5(ROI,37:48)-mean(VelISI_3Dmat_meanISI5(ROI,35:36))...
            == max(VelISI_3Dmat_meanISI5(ROI,37:48)-mean(VelISI_3Dmat_meanISI5(ROI,35:36))...
            ));
        peakAnalysisCell_perISIOff{i,1}(ROI,1).latency = ...
            peakAnalysisCell_perISIOff{i,1}(ROI,1).latency(1,1)*timebin;
        %          = findpeaks(diff(VelISI_3Dmat_meanISI5(ROI,36:48)),'Npeaks',1,...
        %          'WidthReference', 'halfheight');
    end
end
for i= 1%:size(idxLightsOn,2)
    for ROI= 1:nROIs
        if isempty (peakAnalysisCell_perISIOff{i, 1}(ROI,1).amplitude)
            peakAnalysisCell_perISIOff{i, 1}(ROI,1).amplitude = nan;
            peakAnalysisCell_perISIOff{i, 1}(ROI,1).latency = nan;
        end
    end
end
...
peakAnOn_perISI = cell(size(idxLightsOn,2)/5,1);
for g=1:length(groups)-2                % group number
    for i=1%:size(idxLightsOn,2)         % light number
        peakAnOn_perISI{g,1}(i,1)= mean(vertcat...
            (peakAnalysisCell_perISI{i,1}(groups{1,g}).amplitude),'omitnan');
        peakAnOn_perISI{g,1}(i,2)= mean(vertcat...
            (peakAnalysisCell_perISI{i,1}(groups{1,g}).latency),'omitnan');
        
        peakAnOn_perISI{g,1}(i,3)= std (vertcat...
            (peakAnalysisCell_perISI{i,1}(groups{1,g}).amplitude),[],'omitnan')...
            /sqrt(length(groups{1,g}));
        peakAnOn_perISI{g,1}(i,4)= std (vertcat...
            (peakAnalysisCell_perISI{i,1}(groups{1,g}).latency),[],'omitnan')...
            /sqrt(length(groups{1,g}));
    end
end

peakAnOff_perISI = cell(size(idxLightsOn,2)/5,1);
for g=1:length(groups)-2
    for i=1%:size(idxLightsOn,2)
        peakAnOff_perISI{g,1}(i,1)= mean(vertcat...
            (peakAnalysisCell_perISIOff{i,1}(groups{1,g}).amplitude),'omitnan');
        
        peakAnOff_perISI{g,1}(i,2)= mean(vertcat...
            (peakAnalysisCell_perISIOff{i,1}(groups{1,g}).latency),'omitnan');
        peakAnOff_perISI{g,1}(i,3)= std (vertcat...
            (peakAnalysisCell_perISIOff{i,1}(groups{1,g}).amplitude),[],'omitnan')...
            /sqrt(length(groups{1,g}));
        peakAnOff_perISI{g,1}(i,4)= std (vertcat...
            (peakAnalysisCell_perISIOff{i,1}(groups{1,g}).latency),[],'omitnan')...
            /sqrt(length(groups{1,g}));
    end
end
...
for i= 1%:size(idxLightsOn,2)/5
    ...
    Vec = cell(1,1);
    for j=1%:5
%     Vec{i,1}(:,j) = ([peakAnalysisCell_perISI{i, 1}.latency]);                         % something is wrong works only for the 5 min ISI
    Vec = ([peakAnalysisCell_perISI{i, 1}.latency])';                         % something is wrong works only for the 5 min ISI
    end

    %Vec2 = mean(Vec{i,1},2);
    b= boxchart(Vec([groups{1,1:3}],:),'BoxWidth',.5,'BoxFaceAlpha',0.5,'LineWidth',.5,'MarkerStyle','none',...
        'GroupByColor',group_category([groups{1,1:3}],:));
    set(b,{'BoxFaceColor'},C1(1:3))

    h = text(1,2.25,{num2str(ranksum(Vec(group1),Vec(group2))) ;...           % change the number of test done manually
        num2str(ranksum(Vec(group1),Vec(group3)))}); ;...
%         num2str(ranksum(Vec(group1),Vec(group4)))});
set(h,'rotation',270)
ylim([0 3]);
% xlim([0 5]);
xticks([]);
set(gca,'Ygrid','off','Ycolor','none','FontSize',12)
box off;
...
for i= 1%:size(idxLightsOn,2)/5
    ...
    for j=1%:5
%     Vec{i,1}(:,j) = ([peakAnalysisCell_perISI{i, 1}.latency]);                         % something is wrong works only for the 5 min ISI
    Vec = ([peakAnalysisCell_perISI{i, 1}.amplitude])';                         % something is wrong works only for the 5 min ISI
    end
    ...
    h = text(1,45,{num2str(ranksum(Vec(group1),Vec(group2))) ;...           % change the number of test done manually
        num2str(ranksum(Vec(group1),Vec(group3)))});
%         num2str(ranksum(Vec(group1),Vec(group4)))});
set(h,'rotation',270)
ylim([0 60]);
...
for i= 1%:size(idxLightsOn,2)/5
    ...
    for j=1%:5
%     Vec{i,1}(:,j) = ([peakAnalysisCell_perISI{i, 1}.latency]);                         % something is wrong works only for the 5 min ISI
    Vec = ([peakAnalysisCell_perISIOff{i, 1}.amplitude])';                         % something is wrong works only for the 5 min ISI
    end
    ...
    h = text(1,20,{num2str(ranksum(Vec(group1),Vec(group2))) ;...           % change the number of test done manually
        num2str(ranksum(Vec(group1),Vec(group3)))});
set(h,'rotation',270)
ylim([0 20]);
% xlim([0 5]);
