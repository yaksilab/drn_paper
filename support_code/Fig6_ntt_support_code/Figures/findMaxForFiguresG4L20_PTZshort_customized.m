...
for g=1:length(groups)-1                % group number 
...
for i= 1%:size(idxLightsOn,2)/5
    nexttile
    Vec = [peakAnalysisCell{i, 1}.amplitude]';
    b= boxchart(Vec([group1 group2 group3 group4],:),...
        'BoxFaceAlpha',0.5,'LineWidth',.5,'MarkerStyle','none','GroupByColor',group_category([group1 group2 group3 group4],:));
    set(b,{'BoxFaceColor'},C1)
    
    h = text(1,90,{num2str(ranksum(Vec(group1),Vec(group2))) ;...           % change the number of test done manually
...
for i= 1%:size(idxLightsOn,2)/5
    nexttile
    Vec = [peakAnalysisCell{i, 1}.latency]';
    b= boxchart(Vec([group1 group2 group3 group4],:),...
        'BoxFaceAlpha',0.5,'LineWidth',.5,'MarkerStyle','none','GroupByColor',group_category([group1 group2 group3 group4],:));
    set(b,{'BoxFaceColor'},C1)
    
    h = text(1,1.6,{num2str(ranksum(Vec(group1),Vec(group2))) ;...           % change the number of test done manually
        num2str(ranksum(Vec(group1),Vec(group3))) ;...
        num2str(ranksum(Vec(group1),Vec(group4)))}) ;...
%             num2str(ranksum(Vec(group1),Vec(group2))) ]);
set(h,'Rotation',270);
...
% for i= 1:size(idxLightsOn,2)/5
%     nexttile
%     Vec = [peakAnalysisCellOff{i, 1}.amplitude]';
%     b= boxchart(Vec([group1 group2 group3 group4],:),...
%         'GroupByColor',group_category([group1 group2 group3 group4],:));
%     set(b,{'BoxFaceColor'},C1)
%     
...
% for i= 1:size(idxLightsOn,2)/5
%     nexttile
%     Vec = [peakAnalysisCellOff{i, 1}.latency]';
%     b= boxchart(Vec([group1 group2 group3 group4],:),...
%         'GroupByColor',group_category([group1 group2 group3 group4],:));
%     set(b,{'BoxFaceColor'},C1)
%     
