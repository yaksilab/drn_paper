function [collected_yposition, change_yposition, marker_stim, stim_times, yposition] = ...
    AO_plot_ypos_average_and_change...
    (all_fish, duration, group, name, folder_path_save, figures_subfolder, single_ploting, baseline_sec)
...
%
%   Syntax:
%       [collected_yposition,change_yposition, marker_stim] = AO_plot_ypos_average_and_change
% (all_fish, duration, group, name, folder_path_save, figures_subfolder, single_ploting)
%    
%   Inputs:
...
%
%   Outputs:
%       collected_yposition - array for each fish with average y pos per trial in the group 
% with dim time x
%       fish x stim
%       change_yposition - array for each fish with change in y pos per trial in the group 
% with dim time x
%       fish x stim
...
stim_times = all_fish{group(1), 1}.timeStampStim(find(~contains(...
    all_fish{group(1), 1}.stimInfo, "VIB_0")));
...
len_y_trace = size(-y(t>floor(stim_times(end))-duration & t<floor(stim_times(end))+duration),1)-10;
...
yposition = nan([len_y_trace, size(group,1), length(stim_times)]);
for stim = 1:length(stim_times)
    for fish = 1:size(group,1)
        if ismember(group(fish),lowerRow)
           curr_stim_times = all_fish{group(fish), 1}.timeStampStim(find(~contains(all_fish...
               {group(fish), 1}.stimInfo, "VIB_0")));
       ...
       % kadir
       while length(temp_y) < len_y_trace
           temp_y = [temp_y; temp_y(end)];
       end
       collected_yposition(:, fish, stim) = temp_y(1:len_y_trace);
       
       yposition(:, fish, stim) = temp_y(1:len_y_trace);

       change_yposition(:, fish, stim) = abs(temp_y(1:len_y_trace)) - abs(nanmean(temp_y...
           (marker_stim - (baseline_sec*15):marker_stim), 1)); % 2s average before 2s times framerate
    end
end
...
%     subplot(round(length(stim_types)/2),2,stim)
    ...
    H=shadedErrorBar(x_values, squeeze(nanmean(nanmean(collected_yposition...
        (:,:,current_stim_ind),3),2)),squeeze(nanstd(nanmean(collected_yposition...
...
saveas(gcf, fullfile(folder_path_save, figures_subfolder, name, [name, ...
    '_avg_over_stim_fish_yposition.png'])); 
close; 

% now the change % kadir: ave
figure('units','centimeters','Position',[2 2 20 8])
...
%     subplot(round(length(stim_types)/2),2,stim)
    subplot(1,length(stim_types),stim)
    H=shadedErrorBar(x_values, squeeze(nanmean(nanmean(change_yposition(:,:,current_stim_ind),3),2)),...
        squeeze(nanstd(nanmean(change_yposition(:,:,current_stim_ind),3),0,2)/sqrt(size(group,1))), ...
        'lineProps','k');
%     current_stim_index = find(all_fish{1,1}.t >= stim_times(stim))
    H.mainLine.LineWidth = 1; 
    hold on
...
saveas(gcf, fullfile(folder_path_save, figures_subfolder, name, [name, ...
    '_change_over_stim_fish_yposition.png'])); 
close;
if single_ploting
    for fish = 1:size(group,1)
            ...
            %     subplot(round(length(stim_types)/2),2,stim)
                ...
                if ismember(fish,lowerRow)
                    H=shadedErrorBar(x_values, squeeze(nanmean(collected_yposition(:,fish,...
                        current_stim_ind),3)),squeeze(nanstd(collected_yposition(:,fish,...
                        current_stim_ind),0,3)/sqrt(length(current_stim_ind))), 'lineProps','k');
    ...
    %                 ylim([-280 -140])
            ...
            saveas(gcf, fullfile(folder_path_save, figures_subfolder, name, [name, ' Fish_',num2str...
                (all_fish{group(fish), 1}.realNum), '_average_ypos_per_stim.png'])); 
            close; 
