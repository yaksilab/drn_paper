function [all_fish] = aj_dvvComputation2_43Hz_20210321 (all_fish,cfg,flds)
for k=1:6
    all_fish        = results.(flds{k,1});
    nROIs           = length(results.(flds{k,1}));
    ...
    for ROI = 1:nROIs
        ...
        velVec = all_fish{ROI,1}.binnedVel_2_43Hz; %results.PTZ_s{1, 1}.binnedVel_2_43Hz
        
        if strcmp(cfg.V0method, 'trad')
            if k==1
                meanBaseline = mean(velVec(:,1:600),'omitnan');
            else 
                meanBaseline = mean(velVec(:,1:3600),'omitnan');
            end 
            ...
            if k==1
                all_fish{ROI, 1}.SponBehaviour.DVV_trad = DVV_trad(:,1:600);
            else 
            ...
            if numFrames > 2*wdw
                parfor j=1:numCells
                    ...
                    for i=wdw+1:numFrames-wdw      % sliding wdw. Start at i = wdw+1 and compute before.
                        temp(i-wdw)=mean(dataSlice(i-wdw:i+wdw),'omitnan'); % for i = wdw+1, take frames from 1:2977 and compute 8 percentile
                    end
