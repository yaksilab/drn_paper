% this file uses the DVV-change to do test on  
...
for i=1:size(idxLightsOn,2)/5
    ...
    for ROI = 1:nROIs
        ...
        offResponseDVV_trad{i,2}(ROI,2) = max (tempCell3{i,2}(ROI,16:20));
    end 
end
...
sgtitle (['In group comparison of ON and OFF response (bin 5 before vs',...
    'max dff after, and 16 vs after stim) whithin 3 sec'])
...
%             'LineWidth',1, 'Color', C1(j,:))
        ...
        errorbar (1:2,mean(onResponseDVV_trad{i,2}(groups{1,j},1:2),'omitnan'),...
            std(onResponseDVV_trad{i,2}(groups{1,j},1:2),[],'omitnan')./...
...
% thus normality can be asumed
    ...
    for j= 1:length(groups)-2
        A(i,j)= signrank(onResponseDVV_trad{i,2}(groups{1,j},1),...
                onResponseDVV_trad{i,2}(groups{1,j},2));
    end 
    ...
    for j=1:length(groups)-2
        plot (1:2,mean(offResponseDVV_trad{i,2}(groups{1,j},1:2),'omitnan'),...
            'LineWidth',1, 'Color', C1(j,:));
        
        errorbar (1:2,mean(offResponseDVV_trad{i,2}(groups{1,j},1:2),'omitnan'),...
            std(offResponseDVV_trad{i,2}(groups{1,j},1:2),[],'omitnan')...
            /sqrt(size(groups{1,j},2)),...
            'Color', C1(j,:),'LineWidth', 2), hold on
    end
    xlim([0 2.5])
    ...
    for j= 1:length(groups)-2
        B(i,j)= signrank(offResponseDVV_trad{i,2}(groups{1,j},1),...
                offResponseDVV_trad{i,2}(groups{1,j},2));
    end 
...
for i=1:size(idxLightsOn,2)/5
    ...
    for ROI = 1:nROIs
        onResponseDVV_trad_diff{i,2}(ROI,1) =  (diff(onResponseDVV_trad{i,2}(ROI,:)));
    end
end
...
for i=1:size(idxLightsOn,2)/5
    ...
    for ROI = 1:nROIs
        offResponseDVV_trad_diff{i,2}(ROI,1) =  (diff (offResponseDVV_trad{i,2}(ROI,:)));
    end
end
...
for i=1:size(idxLightsOn,2)/5
    ...
    for j= 1:length(groups)-3
        A(i,j)=ranksum(onResponseDVV_trad_diff{i,2}(groups{1,j},1),...
            onResponseDVV_trad_diff{i,2}(groups{1,3},1));
    end
    text(1.2,100, num2str(((A(i,:)'))))
    
    %% off response
    subplot (2,size(idxLightsOn,2)/5,i+4)
    boxplot (offResponseDVV_trad_diff{i,2}(:,1),...
    ...
    for j= 1:length(groups)-3
        B(i,j)=ranksum(offResponseDVV_trad_diff{i,2}(groups{1,j},1),...
            offResponseDVV_trad_diff{i,2}(groups{1,3},1));
    end
    
    text(1.2,100, num2str(((B(i,:)'))))
end
