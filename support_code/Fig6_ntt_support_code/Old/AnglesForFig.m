...
for j=5:length(fish_names)-1
    ...
    for i=1
        ...
        elseif strcmp (fish_name, 's280F')
            C2 = [[0 0 0];[[197,27,138]/255];[1 1 1];[1 1 1]];
            tempMat = tempMatS280F;
        end
...
f1=tiledlayout(5,5,'TileSpacing','compact','Padding', 'none');
...
for i=1:length(fields(resultsForFig3))
    ...
    if strcmp(fish_name,'PTZ')||strcmp(fish_name,'PTZ_short')
        imagesc (resultsForFig3.(flds3{i,1}).MuAng([groups{1:4}],:))
        colorbar, colormap(flipud(hot)), caxis([0 pi/2]), box 'off'
    else
        imagesc (resultsForFig3.(flds3{i,1}).MuAng([groups{1:3}],:))
        colorbar, colormap(flipud(hot)), caxis([0 pi/2]), box 'off'
    end
    ...
    MuAngR = nan(nROIs,40);
    for ROI=1:nROIs
        if isnan(mean(resultsForFig3.(flds3{i,1}).MuAng(ROI,:)))
        else
            MuAngR(ROI,:) = resample(resultsForFig3.(flds3{i,1}).MuAng(ROI,:),...
                [1/30:1/30:20],2,1,30,'pchip'); % downsampling to two hz
        end
        %         resultsForFig3.MuAngRs.(flds3{i,1}) = MuAngR;
    end
    %% plot mean of mean - line plot (row 2)
