function [all_fish]=aj_characterizeActivity_20200306 (all_fish,timebin,folder_path_save,fish_name,date_exp,group1,group2,group3, group4, group5, name_group1,name_group2, name_group3, name_group4, name_group5,figures_subfolder, group_category,nROIs,groups,name_groups,exclusionTreshold,transitionTime);
...
for ROI=1:nROIs 
    idxEndAcclimatizationT(ROI,:)= find(diff(all_fish{ROI, 1}.t>=floor(transitionTime))==1)+1;
end
...
for ROI=1:nROIs
    if isempty(all_fish{ROI, 1}.inactivityPeriods)
        idxEndOfAcclimatizationInact(ROI,:) = nan;
    elseif isempty(find (diff(all_fish{ROI, 1}.inactivityPeriods(1,:)...
        >=idxEndAcclimatizationT(ROI,:)),1))
        idxEndOfAcclimatizationInact(ROI,:)  = nan;
    else
        idxEndOfAcclimatizationInact(ROI,:)  = find (diff(all_fish{ROI, 1}.inactivityPeriods(1,:)...
...
for ROI = 1:nROIs
    if isempty(all_fish{ROI, 1}.bouts)
...
for ROI=1:nROIs
    ...
    elseif isempty(find(diff(all_fish{ROI, 1}.bouts(1,:)>=idxEndAcclimatizationT(ROI,:)), 1)) 
        idxEndOfAcclimatizationBouts(ROI,:) = nan; % this line gives nan for the fish that do no move at all during the first hour
    else
        idxEndOfAcclimatizationBouts(ROI,:)  = find (diff(all_fish{ROI, 1}.bouts(1,:)...
        >=idxEndAcclimatizationT(ROI,:)));  
    end 
end
...
for ROI = 1:nROIs
    ...
    elseif isempty(find(diff(all_fish{ROI, 1}.bouts(1,:)>=idxEndAcclimatizationT(ROI,:)), 1)) 
        ...
        boutLengthSpon_tempMatrix (ROI,1)= mean(all_fish{ROI, 1}.bouts(3,1:idxEndOfAcclimatizationBouts(ROI,:)),2);
    end
end
...
sgtitle(['CharactarizationOfActivity ' 'Spontanious behaviour'])
...
% % % % % % % % %         text(0.1,0.5, 'No movement');
% % % % % % % % %     elseif isnan(1:idxEndOfAcclimatizationBouts(ROI,:))
% % % % % % % % %         histogram (all_fish{ROI, 1}.bouts(3,1))
% % % % % % % % %     else
% % % % % % % % %         if idxEndOfAcclimatizationBouts(ROI,1)<5
% % % % % % % % %             text(0.1,0.5,'Not enough data');
% % % % % % % % %         else
% % % % % % % % %             histfit(all_fish{ROI, 1}.bouts(3,1:idxEndOfAcclimatizationBouts(ROI,:)))
% % % % % % % % %         end
% % % % % % % % %     end
% % % % % % % % %     box off
% % % % % % % % %     
...
for ROI = 1:nROIs
    ...
    elseif isempty(find(diff(all_fish{ROI, 1}.bouts(1,:)>=idxEndAcclimatizationT(ROI,:)), 1)) 
        ...
        boutLengthConditional_tempMatrix (ROI,1)= mean(all_fish{ROI, 1}.bouts(3,idxEndOfAcclimatizationBouts(ROI,:)+1:end),2);
    end
end
...
% % % % %     if isnan(idxEndOfAcclimatizationBouts(ROI))
% % % % %         text(0.1,0.5, 'No movement');
% % % % %     elseif isempty((all_fish{ROI, 1}.bouts(3,idxEndOfAcclimatizationBouts(ROI,:)+1:end)))...
% % % % %             || size((all_fish{ROI, 1}.bouts(3,idxEndOfAcclimatizationBouts(ROI,:)+1:end)),2)<5
% % % % %         histogram(all_fish{ROI, 1}.bouts(3,idxEndOfAcclimatizationBouts(ROI,:)+1:end))
% % % % %     else
% % % % %         histfit(all_fish{ROI, 1}.bouts(3,idxEndOfAcclimatizationBouts(ROI,:)+1:end))
% % % % %     end
% % % % %     box off
% % % % %     
% % % % %     if ismember(ROI,group1)
