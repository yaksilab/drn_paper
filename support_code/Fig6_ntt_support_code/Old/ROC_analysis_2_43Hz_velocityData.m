...
for i= 1:size(idxLightsOn,2)/5
    for ROI=1:nROIs
        if isnan(onResponseVdiff{i, 2}(ROI,1))
            ROCinputON{i,1} (ROI,1) = 0;
        elseif isinf(onResponseVdiff{i, 2}(ROI,1))
            ROCinputON{i,1} (ROI,1) = max(max(ROCinputON{i,1}));
        else
        ROCinputON{i,1} (ROI,1) = onResponseVdiff{i, 2}(ROI,1);
        end         
        if ismember(ROI,group2) || ismember (ROI,group3)
            ROCinputON{i,1}(ROI,2) = 0;
        elseif ismember (ROI,group4) || ismember (ROI,group5)
            ROCinputON{i,1}(ROI,2) = nan;
        else
...
for i=1:length(ROCinputON)
ROCinputON{i,1} = rmmissing(ROCinputON{i,1});
% rmmissing
end 
...
for i= 1:size(idxLightsOn,2)/5
    for ROI=1:nROIs
        if isnan(offResponseVdiff{i, 2}(ROI,1))
        ROCinputOFF{i,1} (ROI,1) = 0;    
        else
        ROCinputOFF{i,1} (ROI,1) = offResponseVdiff{i, 2}(ROI,1);
        end 
        if ismember(ROI,group2) || ismember (ROI,group3) 
            ROCinputOFF{i,1}(ROI,2) = 0;
        elseif ismember (ROI,group4) || ismember (ROI,group5)
            ROCinputOFF{i,1}(ROI,2) = nan;
        else
            ROCinputOFF{i,1} (ROI,2) = 1;
        end
    end
end
for i=1:length(ROCinputOFF)
ROCinputOFF{i,1} = rmmissing(ROCinputOFF{i,1});
% rmmissing
end 
...
CriteriaMatDVV = horzcat((1:.1:25)',(0:.1:24)');
...
for k=1:length (CriteriaMatDVV)
    for i= 1:size(idxLightsOn,2)/5
        for ROI =1:nROIs
            if onResponseVdiff{i, 2}(ROI,1) >= CriteriaMatDVV (k,1)...
                    && onResponseVdiff{i, 2}(ROI,1) >= CriteriaMatDVV (k,2)
...
confusionMatV = cell(size(idxLightsOn,2)/5,1); % is needed to see the responses

for i=1:size(idxLightsOn,2)/5
    for threshold=1:length(CriteriaMatDVV)
        confusionMatV{i,1}(threshold,1) = sum(predGeno5onAndOff{i,1}...
            ([group1],threshold)==1,1)/... TP; true positive
            (sum (predGeno5onAndOff{i,1}([group1],threshold)==1,1)+ ... TP
            sum (predGeno5onAndOff{i,1}([group1],threshold)==0,1)); % false negative, calculates the sensitivity for the threshold value
        
        confusionMatV{i,1}(threshold,2)= (sum(predGeno5onAndOff{i,1}...
            ([group2,group3],threshold)==0,1)/... TN; true negative
            (sum(predGeno5onAndOff{i,1}([group2,group3],threshold)==0,1) + ... TN
            sum(predGeno5onAndOff{i,1}([group2,group3],threshold)==1,1))); % false positive; FP, calculates the spesificity for the threshold value
    end
end
...
for i=1:size(idxLightsOn,2)/5
    ...
    rocfit = fit(1-(confusionMatV{i,1}(:,2)),confusionMatV{i,1}(:,1),ft,opts);
    
    plot (rocfit,'r',1-(confusionMatV{i,1}(:,2)),confusionMatV{i,1}(:,1), '-ob')
...
saveas (f19, [folder_path_save,filesep,figures_subfolder,filesep,...
    fish_name,date_exp '_ROC_2Parameter_V'  formats{1,f}]);
end 
