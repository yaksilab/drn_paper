...
peakAnalysisCellAng = cell (size(idxLightsOn,2),1); % for DVV_trad
...
for ROI=1:nROIs
    peakAnalysisCellAng(ROI,1).amplitude = ...
        max(MuAngR{i, 6}(ROI,7:18)-mean(MuAngR{i, 6}(ROI,1:6),...
        'omitnan'));
    peakAnalysisCellAng(ROI,1).latency = (find...
        ((MuAngR{i, 6}(ROI,7:18)-mean(MuAngR{i, 6}(ROI,1:6)))...
        == max((MuAngR{i, 6}(ROI,7:18))-mean(MuAngR{i, 6}...
        (ROI,1:6)))))*timebin;
    peakAnalysisCellAng{i,1}(ROI,1).latency = ...
        peakAnalysisCellAng{i,1}(ROI,1).latency(1,1)*timebin;
    %          = findpeaks(diff(MuAngR{i, 6}(ROI,12:24)),'Npeaks',1,...
...
for i= 1:size(idxLightsOn,2)
    for ROI= 1:nROIs
        if isempty (peakAnalysisCellAng{i, 1}(ROI,1).amplitude)
            peakAnalysisCellAng{i, 1}(ROI,1).amplitude = nan;
            peakAnalysisCellAng{i, 1}(ROI,1).latency = nan;
        end 
    end 
end 

peakAnalysisCellAngOff = cell (size(idxLightsOn,2),1); % for DVV_trad

for i=1:size(idxLightsOn,2)
    for ROI=1:nROIs
        peakAnalysisCellAngOff{i, 1}(ROI,1).amplitude =...
            max(MuAngR{i, 6}(ROI,29:39)-mean(MuAngR{i, 6}(ROI,27:28),...
            'omitnan'));
        peakAnalysisCellAngOff{i,1}(ROI,1).latency =...
            find(MuAngR{i, 6}(ROI,29:39)-mean(MuAngR{i, 6}(ROI,27:28))...
            == max(MuAngR{i, 6}(ROI,29:39)-mean(MuAngR{i, 6}(ROI,27:28))...
            ));
        peakAnalysisCellAngOff{i,1}(ROI,1).latency = ...
            peakAnalysisCellAngOff{i,1}(ROI,1).latency(1,1)*timebin;
%          = findpeaks(diff(MuAngR{i, 6}(ROI,36:48)),'Npeaks',1,...
%          'WidthReference', 'halfheight');
    end
end
...
for i= 1:size(idxLightsOn,2)
    for ROI= 1:nROIs
        if isempty (peakAnalysisCellAngOff{i, 1}(ROI,1).amplitude)
            peakAnalysisCellAngOff{i, 1}(ROI,1).amplitude = nan;
            peakAnalysisCellAngOff{i, 1}(ROI,1).latency = nan;
        end 
    end 
end 
...
for g=1:length(groups)-2                % group number 
    for i=1:size(idxLightsOn,2)         % light number
        peakAnOn{g,1}(i,1)= mean(vertcat...
            (peakAnalysisCellAng{i,1}(groups{1,g}).amplitude),'omitnan');
        peakAnOn{g,1}(i,2)= mean(vertcat...
            (peakAnalysisCellAng{i,1}(groups{1,g}).latency),'omitnan');
...
for g=1:length(groups)-2
    for i=1:size(idxLightsOn,2)
        peakAnOff{g,1}(i,1)= mean(vertcat...
            (peakAnalysisCellAngOff{i,1}(groups{1,g}).amplitude),'omitnan');
        
        peakAnOff{g,1}(i,2)= mean(vertcat...
            (peakAnalysisCellAngOff{i,1}(groups{1,g}).latency),'omitnan');
...
for i= 1:size(idxLightsOn,2)/5
    ...
    Vec = cell(5,1);
    for j=1:5
    Vec{i,1}(:,j) = ([peakAnalysisCellAng{j, 1}.amplitude]);                         % something is wrong works only for the 5 min ISI
    end 
...
for i= 1:size(idxLightsOn,2)/5
    ...
    Vec = cell(5,1);
    for j=1:5
    Vec{i,1}(:,j) = ([peakAnalysisCellAng{j, 1}.latency]);                         % something is wrong works only for the 5 min ISI
    end 
...
for i= 1:size(idxLightsOn,2)/5
    ...
    Vec = cell(5,1);
    for j=1:5
    Vec{i,1}(:,j) = ([peakAnalysisCellAngOff{i, 1}.amplitude]);                         % something is wrong works only for the 5 min ISI
    end 
...
for i= 1:size(idxLightsOn,2)/5
    ...
    Vec = cell(5,1);
    for j=1:5
    Vec{i,1}(:,j) = ([peakAnalysisCellAngOff{i, 1}.latency]);                         % something is wrong works only for the 5 min ISI
    end 
