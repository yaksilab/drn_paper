function [all_fish] = aj_evokedBdeepAnalysis_3groups...
...
boxplot (tempMat3([group1 group2 group3],:),...
    string(group_category ([group1 group2 group3],:)),'PlotStyle','compact')
    % G-variable
box ('off')
set(gca, 'TickDir','out')
xlim ([.5 3.5])
...
for i=1:length(tests)
    [tests(i,1),~] = ranksum(tempMat3(groups{1,i}),tempMat3(groups{1,3}));
end
...
%     LightStimTest_numresponders_DVV % ResponseDVV_trad_tests %% just of responses 
...
ylabel(f13,'V')
f13.Padding = 'none';
A =  nan(length(groups)-2,4);
B =  nan(length(groups)-2,4); 
...
for i=1:size(idxLightsOn,2)/5
    ...
    box 'off'
       ...
       text(0.2,mean(onResponseV{i,2}(groups{1,3},1:2),'all','omitnan'),...
       num2str(((A(i,:)'))))
...
for i=1:size(idxLightsOn,2)/5
    ...
    %% test 
    A =  nan(length(groups)-3,3); % in group comparison, 
    ...
    for j= 1:length(groups)-3
        A(i,j)=ranksum(onResponseVdiff{i,2}(groups{1,j},1),...
            onResponseVdiff{i,2}(groups{1,3},1));
    end
    ...
    boxplot (offResponseVdiff{i,2}(:,1),...
        group_category,'Colors', C1,'OutlierSize',.2,'PlotStyle','compact'),
    ...
    for j= 1:length(groups)-3
        B(i,j)=ranksum(offResponseVdiff{i,2}(groups{1,j},1),...
            offResponseVdiff{i,2}(groups{1,3},1));
    end
    
    text(1.2,mean(onResponseV{i,2}(groups{1,3},1:2),'all','omitnan'),...
       num2str(((B(i,:)'))))
end
...
for i=5:9
    ...
    for ROI=1:nROIs
        ...
        temp12   = (velocity(...
            t >= floor(timeLight(idxLightsOn(ROI,i))) &...
            t <= floor(timeLight(idxLightsOn(ROI,i))+284) )); %% til ten seconds after
...
for i=5:9 % for the 5 min ISI
    ...
    for ROI=1:nROIs
        ...
        temp12   = (x(...
            ...
            t <= (timeLight(idxLightsOn(ROI,i))+284))-min(x)); %% til ten seconds after
        ...
        temp4   = (y(...
            t >= (timeLight(idxLightsOn(ROI,i))) &...
            t <= (timeLight(idxLightsOn(ROI,i))+284))- min(y)); %% til ten seconds after
