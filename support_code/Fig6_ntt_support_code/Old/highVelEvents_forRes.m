function [results]=highVelEvents_forRes(results,flds)
...
for k=1:length(flds(1:4,:))
        all_fish                    =   results.(flds{k,1});
        nROIs                       =   length(all_fish);
        
    for j= 1:length(highVel_thresholds)
        inactivityDefinition        =   1;                                      % Time of inactivity (in sec) needed for definition of sleep/inactivity, according to https://www.frontiersin.org/articles/10.3389/fncir.2013.00058/full
        ...
        for ROI=1:nROIs
            ...
            %load variables
            meanVelbinned   = all_fish{ROI,1}.binnedVel_2_43Hz; 
            ...
            if isnan(mean(all_fish{ROI,1}.bV_E4sd(:,:),2,'omitnan'))
                ...
                for i=2:length(dt)
                    ...
                    elseif binActivityInfo==0 && i==length(dt) && (timeInactive >= inactivityDefinition)
                        ...
                        if(timeInactive >= inactivityDefinition) %asks if time of inactivity is above threshold (here: 1 min)
                            lowSpeedPeriods([1:3],K)=[bin_go_to_sleep; bin_wake_up-1; timeInactive]; %if so, save bins when  goes to sleep and wakes up + time of inactivity
                            K=K+1;
                            2;
                        end
                    else
                    ...
                    elseif (binActivityInfo==1 && i==length(dt)) && timeActive >= activityDefinition
                        timeInactive=timeInactive+dt(i);
                        bouts([1:3],L) =[bin_wake_up; bin_go_to_sleep-1; timeActive]; % save bout information.
                        L=L+1;
                        2;
                    else
