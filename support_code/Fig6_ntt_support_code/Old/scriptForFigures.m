...
b = boxchart(mean(matMeanVelTemp([group1 group2],:),...
    2,'omitnan'),'BoxFaceAlpha',0.5,'LineWidth',.5,'MarkerStyle',...
    'none','GroupByColor',group_category([group1 group2...
    ]));  %boxplot of median velocity for the groups
...
% box('off')
h=text(1,5,num2str([ranksum(bV_E4sd_spontanious(group1),...
    bV_E4sd_spontanious(group2))]));...
...
set(h,'Rotation',90);
% ylabel('Mean swim velocity (mm/s)','FontSize',12)
set(gca,'Ycolor','none');
...
forFigure2_heatmapEvoked_customized
%%
findMaxForFiguresG4L20
...
for i=1
    if strcmp (fish_name, 'PTZ_short') || strcmp (fish_name, 'gabra1') ||...
        ...
        % heatmap evoked behaviour and lineplot 2.43 Hz
        
        forFigure2_heatmapEvoked%_customized
        
        % find max
        
        findMaxForFiguresG4L20%_customized                                               % Change group number for plots etc
