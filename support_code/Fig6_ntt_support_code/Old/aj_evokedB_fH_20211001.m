...
for i=1:size(idxLightsOn,2)
    ...
    for ROI=1:nROIs
        ...
        v_2Hz          = all_fish{ROI, 1}.binnedVel_2Hz;
        ...
        temp12   = (velocity(...
            t >= floor(timeLight(idxLightsOn(ROI,i))) &...
            t < floor(timeLight(idxLightsOn(ROI,i))+alfa-1) )); %% til ten seconds after
        
        T11 = reshape(temp11,[1,5]);
        T12 = interp1(1:length(temp12),temp12,(1:1:alfa));
        ...
        T14 = interp1(1:length(temp4),temp4,(1:1:alfa));
        ...
        T18 = interp1(1:length(temp8),temp8,(1:1:alfa));
       ...
       temp5   = (Burst_binary(...
        ...
        T16 = interp1(1:length(temp6),temp6,(1:1:alfa));
        ...
        temp_total9=[T17,T18];
        tempMat{i,6}(ROI,1:size(temp_total9,2)) = temp_total9;
...
tempVelMat = nan(nROIs,1957);

for ROI = 1:nROIs
    VelVec = [];
    VelVec(1,:) = (all_fish{ROI,1}.bV_E4sd...
        (1,round(all_fish{ROI, 1}.timestampLight(idxLightsOn(ROI,1)-600))...
        :round(all_fish{ROI, 1}.timestampLight(idxLightsOff(ROI,5))+120)...
        ));
    
    tempVelMat (ROI,:) = interp1(1:length(VelVec),VelVec,(1:1:1957)); 
end
...
imagesc (tempVelMat([groups{1,1:9}],:))
title ('Velocity heatmap, S, raw')
colormap ((hot))
colorbar
box off
set(gca,'TickDir','out')
...
tempMat2 = (tempVelMat([groups{1,1:9}],:));
tempMat3 = mean (tempVelMat,2,'omitnan');

f5= figure;
f5.WindowState = 'Maximized';
subplot (2,6,[1:2,7:8])
boxplot (tempMat3([groups{1,1:9}],:),...
    string(group_category (1,[groups{1,1:9}])),...
    'PlotStyle','compact') % G-variablebox ('off')
...
for k=1:size(idxLightsOn,2)/5
    nexttile(k)
    imagesc(tempCell{k,2}(sort([groups{1,1:6}]),:))
...
for j=1:length(groups)-1
    boundedline(1:size(tempCell{k,2},2),...
        mean(tempCell{k,2}(groups{1,j},:)),...
        std((tempCell{k,2}(groups{1,j},:)),[],1,'omitnan')./...
        sqrt(length(groups{1,j})),'alpha',...
        'cmap',C1(j,:));
...
for i=1:size(idxLightsOn,2)/5
    tempCell2{i,1} = tempCell{i,2}([group1,group2,group3,group4,group5,group6],:);
end

f2= figure;
f2.WindowState='Maximized';
% sgtitle('raw velocity, mean response per fish, S heatmap')
f2 = tiledlayout(2,5); % Requires R2019b or later
...
for k=1:size(idxLightsOn,2)/5
    ...
    boundedline(1:size(tempCell{k,2},2),...
        ...
        1:size(tempCell{k,2},2),...
        mean(tempCell{k,2}(group5,:)),...
        std((tempCell{k,2}(group5,:)),[],1,'omitnan')...
        ./sqrt(length(group5)),'alpha',...
        1:size(tempCell{k,2},2),...
        mean(tempCell{k,2}(group6,:)),...
        std((tempCell{k,2}(group6,:)),[],1,'omitnan')...
        ./sqrt(length(group6)),'alpha',...
        'cmap',C1);
    
    legend({'', '', '', '','','', name_groups{1,1:6}})
    if k==1
...
f3 = tiledlayout(2,5); % Requires R2019b or later
...
% aj_alternative_DVV_calculation%% Burstduration 
%% Activity proportion
...
tempCell6{2,1} = ("2'ISI");
tempCell6{3,1} = ("1'ISI");
tempCell6{4,1} = ("0.5'ISI");
tempCell6{5,1} = ("0.25'ISI");
...
tempCell7 = cell(5,1);
for j=1:size(idxLightsOn,2)/5
    tempCell7{j,1} = tempCell6{j,2}([group1,group2,group3,group4,group5,group6],:);
end
...
f7 = tiledlayout(2,5); % Requires R2019b or later
...
for k=1:size(idxLightsOn,2)/5
    nexttile
    imagesc(tempCell7{k,1}(1:length([groups{1,1:6}])))
...
for k=1:size(idxLightsOn,2)/5
    ...
    boundedline(...
        ...
        sqrt(length(group4)),'alpha',...
        1:size(tempCell6{k,2},2),...
        mean(tempCell6{k,2}(group5,:),'omitnan'),...
        std((tempCell6{k,2}(group5,:)),[],1,'omitnan')./...
        sqrt(length(group5)),'alpha',...
        1:size(tempCell6{k,2},2),...
        mean(tempCell6{k,2}(group6,:),'omitnan'),...
        std((tempCell6{k,2}(group6,:)),[],1,'omitnan')./...
        sqrt(length(group6)),'alpha',...
        'cmap',C1);
...
tempCell_V_2_43Hz{4,1} = ("0.5''ISI");
tempCell_V_2_43Hz{5,1} = ("0.25''ISI");
...
f1=tiledlayout(2,5); % Requires R2019b or later
...
for k=1:size(idxLightsOn,2)/5
    if k==1
    nexttile
    imagesc(tempCell_V_2_43Hz{k,2}(([groups{1,1:4}]),1:end))
    ...
    set(gca,'TickDir','out'); % The only other option is 'in'
         ...
         nexttile
    imagesc(tempCell_V_2_43Hz{k,2}(([groups{1,1:4}]),1:85))
...
%% Light on response:  V change per fish per stimulus from prior to the stimulus to max velocity
...
f13 = tiledlayout(2,5); % Requires R2019b or later
...
f14=tiledlayout(2,size(idxLightsOn,2)/5);
title (f14,['Between group comparison of ON and OFF response,',...
    'raw V(bin 5 before vs max V after, and 16 vs after stim)',...
    'whithin 5 sec'])
for i=1:size(idxLightsOn,2)/5
    %% on response
    nexttile (i)
...
tempMat4 = cell(5,5);
for i=1:5
    tempMat4{i,1}= ['Stimulation number' num2str(i)];
    ...
    for ROI=1:nROIs
        ...
        temp_total1=[T11,T12];
        tempMat4{i,2}(ROI,1:size(temp_total1,2)) = temp_total1;
        ...
        temp_total3=[T13,T14];
        tempMat4{i,3}(ROI,1:size(temp_total3,2)) = temp_total3;
       ...
       temp5   = (BurstDur(...
        ...
        temp_total8=[T15,T16];
        tempMat4{i,4}(ROI,1:size(temp_total8,2)) = temp_total8;
        ...
        temp_total8=[T17,T18];
        tempMat4{i,5}(ROI,1:size(temp_total8,2)) = temp_total8;
        ...
        temp_total9=[T19,T20];
        tempMat4{i,6}(ROI,1:size(temp_total9,2)) = temp_total9;
...
tempMat6 = cell(5,3);
for i=1:5 % for the 5 min ISI
    tempMat6{i,1}= ['Stimulation number' num2str(i)];
    ...
    for ROI=1:nROIs
        ...
        temp_total1=[T11,T12];
        tempMat6{i,2}(ROI,1:size(temp_total1,2)) = temp_total1;
        ...
        temp_total3=[T13,T14];
        tempMat6{i,3}(ROI,1:size(temp_total3,2)) = temp_total3;
...
f16.Padding = 'none';
D = [18 18 25 40]; % fish in group 1, 2, 3, 4
...
% Do not change 
    ...
    ang{i,1}(ROI,:) = abs(smooth(ang{i,1}(ROI,:),12));
...
for i=1:5
    ...
    boundedline(1:size(ang{i,1},2),...
            ...
            sqrt(length(group4)),'alpha',...
            1:size(ang{i,1},2),...
            mean(abs(ang{i,1}(group5,:)),'omitnan'),...
            std(abs(ang{i,1}(group5,:)),[],1,'omitnan')./...
            sqrt(length(group5)),'alpha',...
            1:size(ang{i,1},2),...
            mean(abs(ang{i,1}(group6,:)),'omitnan'),...
            std(abs(ang{i,1}(group6,:)),[],1,'omitnan')./...
            sqrt(length(group6)),'alpha',...
        'cmap',C1) 
    hold on 
...
boundedline(1:size(MuAng,2),...
        ...
        sqrt(length(group4)),'alpha',...
        1:size(MuAng,2),...
        mean(MuAng(group5,:),'omitnan'),...
        std(MuAng(group5,:),[],1,'omitnan')./...
        sqrt(length(group5)),'alpha',...
        1:size(MuAng,2),...
        mean(MuAng(group6,:),'omitnan'),...
        std(MuAng(group6,:),[],1,'omitnan')./...
        sqrt(length(group6)),'alpha',...
    'cmap',C1) 
...
%% calculate sensitivity and spesificity for prediction of genotype/ treatmentgroup from light reaction

% Predict the treatment group based on lights reaction

% ROC_analysis_2_43Hz_velocityData