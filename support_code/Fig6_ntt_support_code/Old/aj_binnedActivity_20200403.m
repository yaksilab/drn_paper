function [all_fish]=aj_binnedActivity_20200403(all_fish,nROIs,timebin,folder_path_save,fish_name,date_exp,figures_subfolder,group1,group2,allgroups,name_group1,name_group2)
...
for ROI=1:nROIs
    ...
    %load variables
    meanVelbinned   = all_fish{ROI,1}.binnedVel;
    ...
    if isnan(nanmean(all_fish{ROI,1}.binnedVel(:,:),2))
        all_fish{ROI,1}.binnedInactivityPeriods   =   nan;
        all_fish{ROI,1}.BinnedBouts               =   nan;
    else
        
        for i=2:length(dt)
            if( (meanVelbinned(i-1) <= sleep_speed_threshold || isnan(meanVelbinned(i-1)) ) && i==2) %starts at first registered speed (at bin 2); registers and start counting if fish is inactive
                binActivityInfo=0; %if inactive, starts to count
                bin_go_to_sleep=1;  %... and register bin where starts to be inactive
            elseif ( meanVelbinned(i-1) > sleep_speed_threshold && i==2)
                binActivityInfo=1;
                bin_wake_up=1;
            end
            
            if( meanVelbinned(i-1) > sleep_speed_threshold && meanVelbinned(i) <= sleep_speed_threshold) %...same as above, but this restarts counting when becomes inactive again
                binActivityInfo=0;
                bin_go_to_sleep=i;
            end
            
            if(( meanVelbinned(i-1) <= sleep_speed_threshold || isnan (meanVelbinned(i-1))) && meanVelbinned(i) > sleep_speed_threshold) %registers if fish becomes active
                binActivityInfo=1; %if so, it the fish is active
                bin_wake_up=i; %... and register this bin as the 'wake up' bin
            end
            
            if( meanVelbinned(i) <= sleep_speed_threshold && i==length(dt) ) %when reaches end of matrix ...
                binActivityInfo=0; %...stop counting....
                bin_wake_up=i; %...and registers as 'wake up', to stop the counting
            elseif ( meanVelbinned(i) > sleep_speed_threshold && i==length(dt) )
                binActivityInfo=1;
                bin_go_to_sleep=i;
            end
            ...
            elseif binActivityInfo==0 && i==length(dt) && (timeInactive >= inactivityDefinition)
                ...
                if(timeInactive >= inactivityDefinition) %asks if time of inactivity is above threshold (here: 1 min)
                    inactivityPeriods([1:3],K)=[bin_go_to_sleep; bin_wake_up; timeInactive]; %if so, save bins when  goes to sleep and wakes up + time of inactivity
                    K=K+1;
                    2;
                end
            else
        ...
        all_fish{ROI,1}.binnedInactivityPeriods   =   inactivityPeriods;
        all_fish{ROI,1}.BinnedBouts               =   bouts;
    end
end
